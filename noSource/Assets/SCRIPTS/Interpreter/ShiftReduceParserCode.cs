...
namespace QUT.Gppg {
 ...
 where TSpan : IMerge<TSpan>, new() {
        ...
        private void DisplayStack() {
            Console.print( "State stack is now:" );
            for (int i = 0; i < StateStack.Depth; i++)
                Console.print( " " + StateStack[i].number );
            Console.print("\n");
        }

        private void DisplayRule( int ruleNumber ) {
            Console.print( "Reducing stack by rule {0}, " + ruleNumber );
        ...
        private void DisplayProduction( Rule rule ) {
            if (rule.RightHandSide.Length == 0)
                ...
                foreach (int symbol in rule.RightHandSide)
                    Console.print( SymbolToString( symbol ) + " " );

            Console.print( "-> " + SymbolToString( rule.LeftHandSide ) );
        }
